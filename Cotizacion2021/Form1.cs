...
namespace Cotizacion2021
{
    public partial class Form1 : MaterialForm
    {
        ...
        public static string FolioPasa { get; set; }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            AutorizaBut.BackColor = ColorTranslator.FromHtml("#06a038");
            ...
            button6.BackColor = ColorTranslator.FromHtml("#049bdc");
        ...
        private void cargaDatosEnvOri(string folio)
        {
            cmd.CommandText = "select * from CotizacionesOriginalOT where folioCotizacion = '" + folio + "'";
            cmd.CommandType = CommandType.Text;
            cmd.Connection = sqlConnection1;
            sqlConnection1.Open();
            reader = cmd.ExecuteReader();
        ...
        private void cargaCotizaORI(string folio)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    dataGridView1.Rows[cont].Cells["Peso1"].Value = reader["Peso"].ToString();
                    dataGridView1.Rows[cont].Cells["Importe"].Value = reader["Importe"].ToString();
                    dataGridView1.Rows[cont].Cells["Observa"].Value = reader["Observaciones"].ToString();
                    dataGridView1.Rows[cont].Cells["Eliminar"].Value = "X";
        ...
        private void elimina()
        {
            ...
            try
            {
                string sql = "delete from cotizacionesdetallesOT where folioCotizacion = @param1";
                SqlCommand cmd = new SqlCommand(sql, sqlConnection1);
        ...
        private void actualiza(string id, string estatus)
        {
            continua = 0;
            try
            {
                string sql = @"update cotizacionesOT set codcliente = @param1, esLocal = @param2, observaciones = @Obs, fecha= @Fecha, idCliente = @idCliente,
                       idDireccion = @idDireccion, idEnvio = @idEnvio, importe = @Importe, Estatus = @Estatus, prioridad = @Prioridad, TipoCliente = @TipoCliente, Notas_Internas = @Notas_internas, Atencion = @Antencion, Solicito = @Solicito,
                Fecha_Entrega = @FechaEntrega, Provee_Material = @Prov, Autoriza1 = @Autoriza1, Usuario_Autoriza1 = @UA1, lista_precios = @Lista
                ...
                cmd.Parameters.AddWithValue("@Antencion", Atencion.Text);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (idDocumentoText.Text.Length != 0)
            {
                ...
                {
                    actualiza(idDocumentoText.Text, "1");
                    if (updateCliente())
                    {
        ...
        {
            cmd.CommandText = "select folioCotizacion+1 as folio from cotizacionesOT order by folioCotizacion desc";
            cmd.CommandType = CommandType.Text;
            cmd.Connection = sqlConnection3;
            sqlConnection3.Close();
            sqlConnection3.Open();
        ...
        private bool guardar()
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@Tipo_Doc", "1");
                cmd.Parameters.AddWithValue("@FolioSuc", textSerie.Text);
                cmd.Parameters.AddWithValue("@FolioOT", "");
                cmd.Parameters.AddWithValue("@Sucursal", comboBox1.Text);
                cmd.Parameters.AddWithValue("@FolioCotizacion", ultFol);
                cmd.Parameters.AddWithValue("@FolioCot", "COT-" + comboBox1.Text + "-" + ultFol);
 ...
 WHERE id_mov = @id_mov", sqlConnection3);
	...
	select @id_doc as siguiente";
        ...
        private void exporta(DataGridView datagridview1)
        {
            ...
            rd.Load(Directory.GetParent(Environment.CurrentDirectory).Parent.Parent.FullName + @"\Cotizacion2021\imprep.rpt");
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            if (idDocumentoText.Text.Length != 0)
            {
                if (dataGridView1.Rows.Count > 0)
                {
                    if (existe(idDocumentoText.Text, "1"))
                    {
                        DialogResult result = MessageBox.Show("¿Deseas aceptar esta cotización?", "AVISO", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);

                        if (result == DialogResult.Yes)
                        {
                            if (Autoriza2(idDocumentoText.Text))
                            {
                                MessageBox.Show("La cotización ha sido aceptada", "AVISO", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                                Limpiar();
                            }
                        }
                        else if (result == DialogResult.No)
                    ...
                    {
                        MessageBox.Show("La cotización no ha sido guardada", "AVISO", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
        ...
        private void inserta(string folio, string Referencia, string unidades, string destina, string guia, string mensa, string cuenta, string cajas, string peso)
        {
            ...
            while (error == 1)
            {
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("@cidconcepto", "3229");
                    cmd.Parameters.AddWithValue("@serie", "PPL");
                    

                    cmd.Parameters.AddWithValue("@Folio", folio);
                    cmd.Parameters.AddWithValue("@Referencia", Referencia);
                    cmd.Parameters.AddWithValue("@TotalUnidades", unidades);
                    cmd.Parameters.AddWithValue("@Usuario", "USUARIOPRUEBA ");
        ...
        private void AbrirCargar(string estatus)
        {
            sucursal = comboBox1.Text;
            CargaCotizacion.estatus = estatus;
            using (CargaCotizacion cc = new CargaCotizacion())
            {
                cc.ShowDialog();
            }
            ...
            if (CargaCotizacion.id_DocumentoPasa != "")
            {
                ...
                textFecha.Text = DateTime.Parse(CargaCotizacion.date1).ToString("dd/MM/yyyy");
                Atencion.Text = CargaCotizacion.AtencionPasa;
                Solicito.Text = CargaCotizacion.SolicitoPasa;
                NIText.Text = CargaCotizacion.NotasI;
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            else if (existe(idDocumentoText.Text, "2"))
            {
                idDocumentoPasa = idDocumentoText.Text;
                Adjunta.FolioCotiza = textFolio.Text;
                using (Adjunta ad = new Adjunta())
        ...
        private void GuardaCotizacion()
        {
            try
            {
                string sql = @"insert into CotizacionesOT values (@folioSuc, @Sucursal, @FolioCotizacion, @FolioCot, 
                @codCliente, @Local, @Atencion, @Solicito, @observa, @fecha, @idCliente, @idDireccion, @idEnvio, @Importe, 'A')
                update reportesamsa.dbo.folios set ultimofolio = @FolioCotizacion where sucursal = @Sucursal";
                SqlCommand cmd = new SqlCommand(sql, sqlConnection1);
                cmd.Parameters.AddWithValue("@folioSuc", textSerie.Text);
                cmd.Parameters.AddWithValue("@Sucursal", comboBox1.Text);
                cmd.Parameters.AddWithValue("@FolioCotizacion", ultFol);
        ...
        private void GuardaDetalles()
        {
            ...
            try
            {
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    try
                    {
                        string sql = @"insert into cotizacionesdetallesOT values (@FolioCotizacion, @CodigoProducto, @Cantidad, @Obs)";
                        SqlCommand cmd = new SqlCommand(sql, sqlConnection1);
                        cmd.Parameters.AddWithValue("@FolioCotizacion", textFolio.Text);
                        ...
                        cmd.Parameters.AddWithValue("@Obs", row.Cells["Observa"].Value.ToString());
        ...
        private void GuardaOriginal()
        {
            ...
            try
            {
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    try
                    {
                        string sql = @"insert into cotizacionesoriginalOT values (@FolioCotizacion, @codigoProducto, 
                        @NombreProducto, @Precio, @Cantidad, @Descuento, @PrecioDescuento, @Peso, 
                        @Importe, @existencia, @Observa, @idCliente, @idDireccion, @idEnvio)";
                        SqlCommand cmd = new SqlCommand(sql, sqlConnection1);
                        cmd.Parameters.AddWithValue("@FolioCotizacion", textFolio.Text);
                        cmd.Parameters.AddWithValue("@CodigoProducto", row.Cells["Codigo"].Value.ToString());
                        ...
                        cmd.Parameters.AddWithValue("@Peso", row.Cells["peso1"].Value.ToString());
        ...
        private bool Autoriza2(string id)
        {
            ...
            {
                Principal.Variablescompartidas.AbrirAceros(sqlConnection3);
                SqlCommand cmd = new SqlCommand(@"Update cotizacionesOT set Autoriza2 = @Autoriza2,  Usuario_Autoriza2 = @UA2,
                  Estatus = '2'  where id_Documento = @id", sqlConnection3);
                cmd.CommandType = CommandType.Text;
